...
CI run reporting 64 tests instead of 83 means exactly that, and must be treated as a
failure. (64 is what `test:unit` yields: it excludes every `*.integration.test.ts`, so
the RLS, keepalive, signup **and** login suites vanish.)
