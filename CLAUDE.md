...
CI run reporting 65 tests instead of 84 means exactly that, and must be treated as a
failure. (65 is what `test:unit` yields: it excludes every `*.integration.test.ts`, so
