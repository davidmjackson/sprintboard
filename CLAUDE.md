...
CI run reporting 59 tests instead of 78 means exactly that, and must be treated as a
failure. (59 is what `test:unit` yields: it excludes every `*.integration.test.ts`, so
