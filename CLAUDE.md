...
CI run reporting 93 tests instead of 115 means exactly that, and must be treated as a
failure. (93 is what `test:unit` yields: it excludes every `*.integration.test.ts`, so
